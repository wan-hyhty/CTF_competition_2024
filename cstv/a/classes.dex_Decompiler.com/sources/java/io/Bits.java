...
class Bits {
    ...
    static char getChar(byte[] b, int off) {
        return (char) ((b[off + 1] & Character.DIRECTIONALITY_UNDEFINED) + (b[off] << 8));
    }

    static short getShort(byte[] b, int off) {
        return (short) ((b[off + 1] & Character.DIRECTIONALITY_UNDEFINED) + (b[off] << 8));
    }

    static int getInt(byte[] b, int off) {
        return (b[off + 3] & Character.DIRECTIONALITY_UNDEFINED) + ((b[off + 2] & Character.DIRECTIONALITY_UNDEFINED) << 8) + ((b[off + 1] & Character.DIRECTIONALITY_UNDEFINED) << 16) + (b[off] << 24);
    }
